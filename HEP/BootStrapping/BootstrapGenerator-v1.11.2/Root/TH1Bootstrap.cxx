...
//______________________________________________________________________________
TH1Bootstrap::TH1Bootstrap(const char *name, const char* title, Int_t nreplica, BootstrapGenerator *boot)
: TNamed(name, title)
, fNReplica(nreplica)
, fOwnGen(kFALSE)
{
...
//______________________________________________________________________________
void TH1Bootstrap::InitOnFileRead()
{
   fOwnGen = kTRUE;
   if (!fGenerator) {
...
//______________________________________________________________________________
void TH1Bootstrap::Append(const TH1 &hext, const Bool_t emptybase, TH1 *hbase)
{
   Int_t nbinsbase = emptybase ? 0 : hbase->GetNbinsX();
   Int_t nxbinsext = hext.GetNbinsX();
   Int_t nybinsext = hext.GetNbinsY(); // In case a TH2 is passed
   Int_t nbinstot  = nbinsbase + nybinsext*nxbinsext;

   hbase->SetBins(nbinstot, 1, nbinstot+1);

   hbase->SetBinContent(0, 0);
   for (Int_t y = 1; y <= nybinsext; ++y) {
      for (Int_t x = 1; x <= nxbinsext; ++x) {
         hbase->SetBinContent(nbinsbase + x + (y-1)*nxbinsext, hext.GetBinContent(x, y));
         hbase->SetBinError(nbinsbase + x + (y-1)*nxbinsext, hext.GetBinError(x, y));
      }
   }
...
//______________________________________________________________________________
Int_t TH1Bootstrap::Fill(Double_t x, Double_t w, UInt_t RunNumber, UInt_t EventNumber, UInt_t mc_channel_number)
{
...
//______________________________________________________________________________
void TH1Bootstrap::AddBinContent(Int_t bin, Double_t w, UInt_t RunNumber, UInt_t EventNumber, UInt_t mc_channel_number)
{
   if (fOwnGen) fGenerator->Generate(RunNumber, EventNumber, mc_channel_number);
   const Int_t *BSWeights = fGenerator->GetWeights();
   for (Int_t i = 0; i < fNReplica; ++i) {
...
//______________________________________________________________________________
#if ROOT_VERSION_CODE >= ROOT_VERSION(5,34,0)
Bool_t TH1Bootstrap::Multiply(TF1 *f1, Double_t c1)
{
   for (Int_t i = 0; i < fNReplica; ++i) {
      GetReplica(i)->Multiply(f1, c1);
   }
   return GetNominal()->Multiply(f1, c1);
}
#else
void TH1Bootstrap::Multiply(TF1 *f1, Double_t c1)
{
   for (Int_t i = 0; i < fNReplica; ++i) {
      GetReplica(i)->Multiply(f1, c1);
   }
   GetNominal()->Multiply(f1, c1);
}
#endif
...
//______________________________________________________________________________
Double_t TH1Bootstrap::GetBootstrapCorel(Int_t bin1, const TH1Bootstrap &h1b, Int_t bin2) const
{
   //if (!IsCompatible(h1b)) {
   ...
   for (Int_t i = 0; i < nbins; i++) {
      for (Int_t j = i; j < nbins; j++) {
         ...
         for (Int_t toy = 0; toy < fNReplica; toy++) {
            (*cov)[i][j] += (GetReplica(toy)->GetBinContent(i+1) - havg->GetBinContent(i+1))*(GetReplica(toy)->GetBinContent(j+1) - havg->GetBinContent(j+1))/fNReplica;
         }
         (*cov)[j][i] = (*cov)[i][j];
      }
   }
...
void TH1Bootstrap::Sumw2(Bool_t flag)
{
   for (Int_t i = 0; i < fNReplica; ++i) {
      GetReplica(i)->Sumw2(flag);
   }
   GetNominal()->Sumw2(flag);
}
#else
void TH1Bootstrap::Sumw2()
{
   for (Int_t i = 0; i < fNReplica; ++i) {
      GetReplica(i)->Sumw2();
   }
...
//______________________________________________________________________________
const BootstrapGenerator* TH1Bootstrap::GetGenerator() const
{
   return fGenerator;
}

//______________________________________________________________________________
const Int_t* TH1Bootstrap::ComputeWeights(UInt_t RunNumber, UInt_t EventNumber, UInt_t mc_channel_number)
{
   if (fOwnGen) fGenerator->Generate(RunNumber, EventNumber, mc_channel_number);
   return fGenerator->GetWeights();
}
...
//______________________________________________________________________________
void TH1Bootstrap::Append(const TH1Bootstrap &hext, const Bool_t emptybase)
{
   // Sanity checks
   Int_t nreplica = hext.GetNReplica();
   if (hext.GetNReplica() != fNReplica) {
      Warning("Append", "The passed TH1Bootstrap has a different number of replicas.");

      nreplica = fNReplica > nreplica ? nreplica : fNReplica;
      Warning("Append", "Using %d replicas, the smaller of the two.", nreplica);
   }

   // Append to local TH1Bootstrap
   Append(*hext.GetNominal(), emptybase, GetNominal());
   for (Int_t i = 0; i < nreplica; ++i) {
      Append(*hext.GetReplica(i), emptybase, GetReplica(i));
   }

   // Delete extra local replicas, if hext had fewer
   for (Int_t i = nreplica; i < fNReplica; ++i) {
      if (GetReplica(i)) delete GetReplica(i);
   }
   fNReplica = nreplica;
}

//______________________________________________________________________________
TH1DBootstrap* TH1Bootstrap::FlattenTo1D()
{
   TString name = TString::Format("%s_flat", GetName());
   TString title = TString::Format("%s_flat", GetTitle());

   TH1DBootstrap *flat = NULL;

   if (fOwnGen) flat = new TH1DBootstrap(name.Data(), title.Data(), 1, 0, 1, 
                                         fNReplica);
   else         flat = new TH1DBootstrap(name.Data(), title.Data(), 1, 0, 1,
                                         fNReplica, fGenerator);

   flat->Append(*this, kTRUE);
...
//______________________________________________________________________________
TH1DBootstrap::TH1DBootstrap(const TH1DBootstrap &h1b)
: TH1Bootstrap(h1b)
{
...
//______________________________________________________________________________
TH1DBootstrap::TH1DBootstrap(const TH1D &h1d,
...
//______________________________________________________________________________
TH1DBootstrap::TH1DBootstrap(const char *name, const char *title, Int_t nxbins, const Float_t *xbins,
                             Int_t nreplica, BootstrapGenerator *boot)
...
//______________________________________________________________________________
TH1DBootstrap::TH1DBootstrap(const char *name, const char *title, Int_t nxbins, const Double_t *xbins,
                             Int_t nreplica, BootstrapGenerator *boot)
: TH1Bootstrap(name, title, nreplica, boot)
{
   fHist = new TH1D(name, title, nxbins, xbins);
...
//______________________________________________________________________________
TH1DBootstrap::TH1DBootstrap(const char *name, const char *title, Int_t nxbins, Double_t xlow, Double_t xup,
                             Int_t nreplica, BootstrapGenerator *boot)
: TH1Bootstrap(name, title, nreplica, boot)
{
...
//______________________________________________________________________________
TH1DBootstrap::TH1DBootstrap(const char *name, const char *title, TH1D *nominal, TH1D **replicas,
                             Int_t nreplica, BootstrapGenerator *boot)
: TH1Bootstrap(name, title, nreplica, boot)
{
   // set the nominal histogram to "nominal"
   if (nominal) {
      fHist = new TH1D(*nominal);
      fHist->SetDirectory(0);
      fHist->SetName(name);
   ...
   if (fNReplica > 0 && replicas) {
      ...
      TString repname = "";
      for (Int_t i = 0; i < fNReplica; ++i) {
         fHistReplica[i] = new TH1D(*replicas[i]);
         fHistReplica[i]->SetDirectory(0);
         repname = TString::Format("%s_rep%d", name, i);
         fHistReplica[i]->SetName(repname.Data());
...
//______________________________________________________________________________
TH1DBootstrap* TH1DBootstrap::Rebin(Int_t ngroup, const char* newname, const Double_t *xbins)
{
   // create a clone of the old histogram if newname is specified
   TH1DBootstrap *hnew = this;
   if ((newname && strlen(newname) > 0) || xbins) {
      hnew = new TH1DBootstrap(*this);
      hnew->SetName(newname);
      if (fOwnGen) {
         hnew->SetGenerator(new BootstrapGenerator(*fGenerator));
      } else {
         hnew->SetGenerator(fGenerator);
      }
   }

   hnew->SetNominal(*static_cast<TH1D*>(GetNominal()->Rebin(ngroup, newname, xbins)));
   hnew->GetNominal()->SetDirectory(0);
   TString repname = "";
   for (Int_t i = 0; i < fNReplica; ++i) {
      repname = TString::Format("%s_rep%d", newname, i);
      hnew->SetReplica(i, *static_cast<TH1D*>(GetReplica(i)->Rebin(ngroup, repname.Data(), xbins)));
...
//______________________________________________________________________________
TH1* TH1DBootstrap::GetNominal()
{ return fHist; }

//______________________________________________________________________________
const TH1* TH1DBootstrap::GetNominal() const
{ return fHist; }
...
//______________________________________________________________________________
const TH1* TH1DBootstrap::GetReplica(Int_t i) const
{ return fHistReplica[i]; }
...
void TH1DBootstrap::SetReplica(Int_t i, const TH1D &h1)
{
   ...
   if (!fHistReplica[i]) fHistReplica[i] = new TH1D(h1);
   else                  h1.Copy(*fHistReplica[i]);
   fHistReplica[i]->SetDirectory(0);
}

//______________________________________________________________________________
//            TH1FBootstrap methods
//______________________________________________________________________________

ClassImp(TH1FBootstrap)

//______________________________________________________________________________
TH1FBootstrap::TH1FBootstrap()
: TH1Bootstrap()
, fHist(NULL)
, fHistReplica(NULL)
{ }
...
//______________________________________________________________________________
TH1FBootstrap::TH1FBootstrap(const TH1FBootstrap &h1b)
: TH1Bootstrap(h1b)
{
   fHist = new TH1F(*h1b.fHist);
   fHist->SetDirectory(0);

   if (fNReplica > 0) fHistReplica = new TH1F*[fNReplica];
   for (Int_t i = 0; i < fNReplica; ++i) {
      fHistReplica[i] = new TH1F(*h1b.fHistReplica[i]);
      fHistReplica[i]->SetDirectory(0);
   }
}

//______________________________________________________________________________
TH1FBootstrap::TH1FBootstrap(const TH1F &h1d,
                             Int_t nreplica, BootstrapGenerator *boot)
: TH1Bootstrap(h1d.GetName(), h1d.GetTitle(), nreplica, boot)
{
   fHist = new TH1F(h1d);
...
//______________________________________________________________________________
TH1FBootstrap::TH1FBootstrap(const char *name, const char *title, Int_t nxbins, const Float_t *xbins,
                             Int_t nreplica, BootstrapGenerator *boot)
: TH1Bootstrap(name, title, nreplica, boot)
{
   fHist = new TH1F(name, title, nxbins, xbins);
   fHist->SetDirectory(0);
   ...
   for (Int_t i = 0; i < fNReplica; ++i) {
      repname = TString::Format("%s_rep%d", name, i);
      fHistReplica[i] = new TH1F(repname.Data(), title, nxbins, xbins);
      fHistReplica[i]->SetDirectory(0);
   }
}

//______________________________________________________________________________
TH1FBootstrap::TH1FBootstrap(const char *name, const char *title, Int_t nxbins, const Double_t *xbins,
                             Int_t nreplica, BootstrapGenerator *boot)
: TH1Bootstrap(name, title, nreplica, boot)
{
   fHist = new TH1F(name, title, nxbins, xbins);
...
//______________________________________________________________________________
TH1FBootstrap::TH1FBootstrap(const char *name, const char *title, Int_t nxbins, Double_t xlow, Double_t xup,
                             Int_t nreplica, BootstrapGenerator *boot)
: TH1Bootstrap(name, title, nreplica, boot)
{
   fHist = new TH1F(name, title, nxbins, xlow, xup);
   fHist->SetDirectory(0);
   ...
   for (Int_t i = 0; i < fNReplica; ++i) {
      repname = TString::Format("%s_rep%d", name, i);
      fHistReplica[i] = new TH1F(repname.Data(), title, nxbins, xlow, xup);
      fHistReplica[i]->SetDirectory(0);
   }
}

//______________________________________________________________________________
TH1FBootstrap::TH1FBootstrap(const char *name, const char *title, TH1F *nominal, TH1F **replicas,
                             Int_t nreplica, BootstrapGenerator *boot)
: TH1Bootstrap(name, title, nreplica, boot)
{
   // set the nominal histogram to "nominal"
   if (nominal) {
      fHist = new TH1F(*nominal);
      fHist->SetDirectory(0);
      fHist->SetName(name);
      fHist->SetTitle(title);
   } else {
      Error("TH1FBootstrap", "Pointer to the nominal histogram not provided");
   }

   // set the array of TH1F histograms to "replicas"
   if (fNReplica > 0 && replicas) {
      fHistReplica = new TH1F*[fNReplica];
      TString repname = "";
      for (Int_t i = 0; i < fNReplica; ++i) {
         fHistReplica[i] = new TH1F(*replicas[i]);
         fHistReplica[i]->SetDirectory(0);
         repname = TString::Format("%s_rep%d", name, i);
         fHistReplica[i]->SetName(repname.Data());
         fHistReplica[i]->SetTitle(title);
       ...
       Error("TH1FBootstrap", "Pointer to the array of replicas not provided");
   }
}
...
//______________________________________________________________________________
TH1FBootstrap* TH1FBootstrap::Rebin(Int_t ngroup, const char* newname, const Double_t *xbins)
{
   // create a clone of the old histogram if newname is specified
   TH1FBootstrap *hnew = this;
   if ((newname && strlen(newname) > 0) || xbins) {
      hnew = new TH1FBootstrap(*this);
      hnew->SetName(newname);
      if (fOwnGen) {
         hnew->SetGenerator(new BootstrapGenerator(*fGenerator));
      } else {
   ...
   hnew->SetNominal(*static_cast<TH1F*>(GetNominal()->Rebin(ngroup, newname, xbins)));
   hnew->GetNominal()->SetDirectory(0);
   TString repname = "";
   for (Int_t i = 0; i < fNReplica; ++i) {
      repname = TString::Format("%s_rep%d", newname, i);
      hnew->SetReplica(i, *static_cast<TH1F*>(GetReplica(i)->Rebin(ngroup, repname.Data(), xbins)));
      hnew->GetReplica(i)->SetDirectory(0);
   }
...
//______________________________________________________________________________
TH1* TH1FBootstrap::GetNominal()
{ return fHist; }

//______________________________________________________________________________
const TH1* TH1FBootstrap::GetNominal() const
{ return fHist; }

//______________________________________________________________________________
TH1* TH1FBootstrap::GetReplica(Int_t i)
{
   if (i >= fNReplica) {
      Error("GetReplica", "Cannot request replica %d out of %d", i, fNReplica);
      return NULL;
...
//______________________________________________________________________________
const TH1* TH1FBootstrap::GetReplica(Int_t i) const
{ return fHistReplica[i]; }

//______________________________________________________________________________
void TH1FBootstrap::SetNominal(const TH1F &h1)
{
   if (!fHist) {
      fHist = new TH1F(h1);
   } else {
...
//______________________________________________________________________________
void TH1FBootstrap::SetReplica(Int_t i, const TH1F &h1)
{
   if (!fHistReplica || i >= fNReplica) return;
   if (!fHistReplica[i]) fHistReplica[i] = new TH1F(h1);
   else                  h1.Copy(*fHistReplica[i]);
   fHistReplica[i]->SetDirectory(0);
}
