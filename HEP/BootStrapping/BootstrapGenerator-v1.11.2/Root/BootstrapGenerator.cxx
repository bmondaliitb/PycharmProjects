...
#ifndef STOCC_H
#include "BootstrapGenerator/stocc.h"
#endif
...
BootstrapGenerator::BootstrapGenerator(const char *name, const char *title, Int_t nrep, Int_t seed)
: TNamed(name, title)
, TArrayI(nrep)
, fNReplica(nrep)
{
...
{
   Set(nrep);
   if (!fStoch) fStoch = new StochasticLib2(seed);
   else         fStoch->RandomInit(seed);
}

void BootstrapGenerator::Generate(UInt_t run, UInt_t event, UInt_t mc_channel_number)
{
   ...
   if ((run !=0 || event != 0) && fNReplica > 0) {
      fSeeds[0] = run; fSeeds[1] = event; fSeeds[2] = mc_channel_number;
