...
public:
   BootstrapGenerator();
   BootstrapGenerator(const char *name, const char *title, Int_t nrep, Int_t seed = 65539);
   BootstrapGenerator(const BootstrapGenerator &bg);
   ~BootstrapGenerator();

   void         Generate(UInt_t run = 0, UInt_t event = 0, UInt_t mc_channel_number = 0);
